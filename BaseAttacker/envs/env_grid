...
class GridWorld_3D_env(Environment):
    ...
    def __init__(self):
        self.shape = (GRID_DIMENSION, GRID_DIMENSION)
        
        self.nS = np.prod(self.shape) #16 --- number of states
        self.nA = 4
        ...
        self.seed()
        self.s = self.initial_state #categorical_sample(self.isd, self.np_random)
    ...
    def reset(self):
        self.s = self.initial_state #categorical_sample(self.isd, self.np_random) #start state
