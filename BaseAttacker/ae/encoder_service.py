...
class EncoderService:
    def __init__(self, config, env: Environment = None, algorithm: Algorithm = None, encoder_type: EncoderType = EncoderType.COMBINED):
        ...
        if algorithm is not None:
            # Policy encoder setup using DEFAULT_KWARGS
            policy_kwargs = {
                "enc_in_size": 32,
                "enc_out_size": 5,
                "dec_in_size": 6,
        ...
        # Validate encoder type requirements
        if encoder_type == EncoderType.POLICY_ONLY and algorithm is None:
            raise ValueError("Algorithm required for policy-only encoder")
        elif encoder_type == EncoderType.ENVIRONMENT_ONLY and env is None:
            raise ValueError("Environment required for environment-only encoder")
        elif encoder_type == EncoderType.COMBINED and (env is None or algorithm is None):
            raise ValueError("Both environment and algorithm required for combined encoder")

    def encode(self, victim_system: 'VictimSystem'):
        if self.encoder_type == EncoderType.POLICY_ONLY:
            return self.policy_encoder.Policy_Embedding(victim_system.algorithm.transitions)
        elif self.encoder_type == EncoderType.ENVIRONMENT_ONLY:
            return self.env_encoder.encode_environment(victim_system.env.altitude)
